...
import { loadActiveWorld } from './worldStateDb.ts';
...
async function renderWorld(displayName: string, role: string) {
  ...
  hud.innerHTML = `<div class="tv-brand">ХРОНИКИ МИРА</div><div class="tv-meta"><span>TV MODE</span><span>${displayName}</span><span>Месяц ${month} / 50</span><span>${gameCode}</span></div><button id="logout" class="logout">Выйти</button>`;
  ...
  info.innerHTML = `<div class="hex-info-kicker">МИР</div><h2>Выберите территорию</h2><p>${gameId ? 'Данные загружены из World State.' : 'Активной партии пока нет. Показан процедурный прототип.'}</p>`;
  ...
  const pickHex = (event: PointerEvent): THREE.Mesh | null => {
    ...
    const hit = raycaster.intersectObjects(scene.children, true).find((item) => item.object instanceof THREE.Mesh && typeof item.object.userData.hexId === 'string');
    return hit?.object instanceof THREE.Mesh ? hit.object : null;
  };
  const formatTerrain = (terrain: WorldHex['terrain']) => ({ plains: 'Равнина', forest: 'Лес', mountain: 'Горы', water: 'Вода', desert: 'Пустыня' })[terrain];
  const updateInfo = (hex: WorldHex) => {
    ...
    info.innerHTML = `<div class="hex-info-kicker">ТЕРРИТОРИЯ</div><h2>${formatTerrain(hex.terrain)}</h2><div class="hex-grid"><span>Координаты</span><strong>${hex.q}, ${hex.r}</strong><span>Высота</span><strong>${hex.elevation.toFixed(2)}</strong><span>Ресурс</span><strong>${resource}</strong><span>Поселение</span><strong>${settlement}</strong><span>Дорога</span><strong>${hex.road ? 'есть' : 'нет'}</strong><span>Река</span><strong>${hex.river ? 'есть' : 'нет'}</strong><span>Тьма</span><strong>${hex.darkness}%</strong></div>`;
  };
  ...
  renderer.domElement.addEventListener('pointerdown', (event) => {
    ...
    dragging = true; moved = false; lastPointer = { x: event.clientX, y: event.clientY }; renderer.domElement.setPointerCapture(event.pointerId);
  });
  renderer.domElement.addEventListener('pointerup', (event) => {
    if (event.button !== 0) return;
    dragging = false; renderer.domElement.releasePointerCapture(event.pointerId);
    if (moved) return;
    const next = pickHex(event); if (!next) return;
    restoreMesh(selected); if (hovered && hovered !== next) restoreMesh(hovered);
    selected = next; highlightMesh(selected, 0xd9f1df, 0.8);
    const hex = hexById.get(String(selected.userData.hexId)); if (hex) updateInfo(hex);
  });
  renderer.domElement.addEventListener('pointerleave', () => { if (!dragging) { restoreMesh(hovered); hovered = null; } });
  renderer.domElement.addEventListener('wheel', (event) => {
    event.preventDefault(); cameraDistance = THREE.MathUtils.clamp(cameraDistance + event.deltaY * 0.015, 13, 42); camera.position.z = cameraDistance; camera.lookAt(camera.position.x, camera.position.y, 0);
  }, { passive: false });
  window.addEventListener('resize', () => { camera.aspect = window.innerWidth / window.innerHeight; camera.updateProjectionMatrix(); renderer.setSize(window.innerWidth, window.innerHeight); });
  document.querySelector<HTMLButtonElement>('#logout')?.addEventListener('click', async () => { await supabase.auth.signOut(); location.reload(); });
  void role;
